...
class TropicalFlow(nn.Module):
    ...
    def sample_discrete(
        ...
        if print_out:
            ...
            print(f"cdf:{cdf}")
            print(f"r:{r}")
            print(f"unnorm_probs:{unnorm_probs}")
            print(f"prob:{prob}")
            print(f"prob sum:{sum(prob)}")
...
class TropicalIntegrator:
    ...
    def train(self, iterations: int, log_interval=100, plt_prog: bool = False):
        ...
        for i in range(iterations):
            samples, (xoh, prob, net_cache) = self.sample(self.batch_size)
        ...
        if plt_prog:
            ...
            # Control should add up to 1 (probability that any channel is chosen)
            control = np.sum(probs, axis=1)
            losses = np.array(losses)*loss_scaling
            log_iters = np.arange(start=log_interval,
            ...
            for ch in range(3):
                ...
                ax.plot(log_iters, probs[:, 2*ch],
                        label=f"Channel{channel_sigs[2*ch]}")
                ax.plot(log_iters, probs[:, 2*ch+1],
                        label=f"Channel{channel_sigs[2*ch+1]}")
                ax.set_title(lbl)
                ax.legend()
                axs[1, 1].plot(log_iters, channel, label=lbl+"x")

            axs[1, 1].plot(log_iters, losses,
                           color='black', label=f"{loss_scaling}*Losses")
            axs[1, 1].plot(log_iters, control, color='black',
                           label="Sum", ls='-.')
            axs[1, 1].legend()

            plt.ylim([0, 1.05])
            filename = "oldprobs_evolution"+datetime.now().strftime("%Y_%m_%d-%H_%M_%S")+".png"
